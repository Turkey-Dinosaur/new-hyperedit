...
export default function ObsidianPanel({ sessionId, onRefreshAssets }: ObsidianPanelProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!prompt.trim() || isSearching || !sessionId) return;
    ...
    try {
      const response = await fetch(`http://localhost:3333/session/${sessionId}/obsidian/search`, {
        method: 'POST',
  ...
  const handleImport = async (videoId: number) => {
    if (importingIds.has(videoId) || !sessionId) return;

    setImportingIds((prev) => new Set(prev).add(videoId));

    try {
      const response = await fetch(`http://localhost:3333/session/${sessionId}/obsidian/import`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="flex flex-col h-full bg-zinc-900/80">
      ...
      <div className="p-4 border-b border-zinc-800/50">
        ...
        <p className="text-xs text-zinc-400">
          Search your video knowledge vault and import clips directly into your editor
        </p>
      </div>

      {/* Searching indicator */}
      {isSearching && (
        <div className="p-3 bg-[#39FF14]/5 border-b border-[#39FF14]/20 flex items-center gap-3">
          <Loader2 className="w-4 h-4 text-[#39FF14] animate-spin flex-shrink-0" />
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t border-zinc-800/50">
        <div className="flex gap-2">
          <input
            ...
            placeholder={sessionId ? 'Search your video vault...' : 'Upload a video first to start a session'}
          ...
          <button
            type="submit"
            disabled={isSearching || !prompt.trim() || !sessionId}
